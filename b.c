...
int execute_command(char *cmd, char *args[], char **env)
{
	...
	/* processus enfant */
	if (pid == 0)
	{/* Exec cmd avec les arguments puis si réeussit,ferme le processus enfant */
		if (execve(cmd, args, env) == -1)
	{/*Si Exec echoue imprime message erreur puis ferme le processus enfant */
		perror("execve");
		exit(EXIT_FAILURE);
	}
	}
	...
	wait(&status);/*Attend fin process enfant et reccup status processus enfant*/
...
int main(int ac, char **av, char **env)
{
	...
	while (getline(&user_input, &bufsize, stdin) != -1)/*attend saisie user */
	{
		...
		if (length > 1)
		{
			...
			full_path = _which(args[0], env);/*teste si args[0] existe dans env*/
			if (full_path)
			{
